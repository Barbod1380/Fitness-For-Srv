...
def _calculate_b31g_allowable_curve(length_range_mm, pipe_diameter_mm, wall_thickness_mm, smys_mpa, method='original'):
    """
    Calculate the maximum allowable depth curve for B31G standards.
    
    This function generates the assessment curves shown in the plot by calculating
    the maximum allowable defect depth for each length according to B31G criteria.
    
    Parameters:
    - length_range_mm: Array of defect lengths in mm
    - pipe_diameter_mm: Pipe diameter in mm
    ...
    Returns:
    - Array of maximum allowable depths in mm
    """
    
    allowable_depths = []
    
    # Calculate flow stress based on method
    if method == 'original':
        flow_stress_mpa = 1.1 * smys_mpa  # Original B31G
    else:
        flow_stress_mpa = smys_mpa + 69.0  # Modified B31G (SMYS + 10 ksi)
    
    # Intact pipe hoop-burst pressure (Barlow's formula)
    P_o_mpa = (2.0 * flow_stress_mpa * wall_thickness_mm) / pipe_diameter_mm
    
    for length_mm in length_range_mm:
        try:
            # Calculate dimensionless length parameter z = L²/(D*t)
            z = (length_mm ** 2) / (pipe_diameter_mm * wall_thickness_mm)
            
            # B31G applicability limits
            if z > 50:  # Beyond Level-1 applicability
                allowable_depths.append(np.nan)
                continue
            
            # Calculate Folias factor based on method
            if method == 'original':
                if z <= 20:  # Short flaw
                    M = np.sqrt(1.0 + 0.8 * z)
                else:  # Long flaw  
                    M = np.sqrt(1.0 + 0.8 * z)  # Simplified for curve generation
            else:  # Modified B31G
            ...
            # For curve generation, we solve for maximum allowable A/A0 ratio
            # that satisfies the B31G equation with a safety factor
            safety_factor = 1.39  # Standard gas pipeline safety factor
            
            # Working backwards from acceptable pressure ratio
            # Pf/P_o should be >= 1/safety_factor for acceptable defect
            min_pressure_ratio = 1.0 / safety_factor
            
            # From B31G: Pf/P_o = (1 - A/A0) / (1 - (A/A0)/M)
            # Solving for A/A0 given minimum pressure ratio
            # This is a quadratic equation in A/A0
            ...
            # Take the smaller positive root (more conservative)
            A_A0_1 = (-b + np.sqrt(discriminant)) / (2*a)
            A_A0_2 = (-b - np.sqrt(discriminant)) / (2*a)
            
            # Choose the appropriate root (between 0 and 1)
            A_A0_max = None
            for root in [A_A0_1, A_A0_2]:
                if 0 <= root <= 1:
            ...
            # Convert A/A0 ratio to maximum allowable depth
            # For simplified calculation, assume A/A0 ≈ 0.85 * d/t (Modified B31G approach)
            max_d_t = A_A0_max / 0.85
            
            # Apply 80% wall thickness limit (B31G applicability)
            max_d_t = min(max_d_t, 0.8)
            
            # Convert to depth in mm
            max_depth_mm = max_d_t * wall_thickness_mm
            
            allowable_depths.append(max_depth_mm)
            
        except Exception as e:
            # For any calculation errors, use NaN
            allowable_depths.append(np.nan)
    
    # Convert to numpy array and handle NaN values
    allowable_depths = np.array(allowable_depths)
    
    # For plotting, replace NaN with previous valid value or limit
    for i in range(len(allowable_depths)):
        if np.isnan(allowable_depths[i]):
            if i > 0 and not np.isnan(allowable_depths[i-1]):
                ...
                # Use 80% wall thickness as conservative limit
